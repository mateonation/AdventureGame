...
function modifyCell(cell,levelnum){
    // UNIQUE ELEMENTS FOR EVERY LEVEL
    spawnpoint=["0/-9","-5/-12"];
    goal=["-10/7","25/-12"];
    doorbutton=["-8/-6","-12/-2"];
    switch(levelnum){
        ...
        // Level 0
        case 0:
            // WALL, LAVA AND DOOR ARRAYS FOR THE LEVEL 0 SHOULD BE ADDED HERE
            wall=[
    ...
    else if(doorbutton[levelnum]===cell.id){
        cell.classList.add('door-button');
    }
    ...
    else if(goal[levelnum]===cell.id){
        cell.classList.add('goal');
    }
    // Add 'cell' class to the cell if it can be assigned
    else{
